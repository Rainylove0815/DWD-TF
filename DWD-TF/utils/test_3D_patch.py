...
def test_all_case_BraTS19(model, image_list, num_classes, patch_size=(96, 96, 64), stride_xy=16, stride_z=4,
                          save_result=True,
    ...
    for image_path in loader:
        ...
        mid_z = prediction.shape[2] // 2
        plt.figure(figsize=(12, 4))
        plt.subplot(131);
        plt.imshow(image[:, :, mid_z], cmap='gray');
        plt.title('image')
        plt.subplot(132);
        plt.imshow(label[:, :, mid_z], cmap='gray');
        plt.title('label')
        plt.subplot(133);
        plt.imshow(prediction[:, :, mid_z], cmap='gray');
        plt.title('pred')
        plt.tight_layout()
        plt.savefig(os.path.join(test_save_path, f'{ith:02d}_slice.png'))
        plt.close()
...
def test_all_case_Pancreas(model, image_list, num_classes, device, patch_size=(96, 96, 64), stride_xy=16, stride_z=4,
                           save_result=True,
                           test_save_path=None, preproc_fn=None, metric_detail=0, nms=0):
    loader = tqdm(image_list) if not metric_detail else image_list
    total_metric = 0.0
    ...
    for image_path in loader:
        ...
        if preproc_fn is not None:
            image = preproc_fn(image)
        prediction, score_map = test_single_case(model, image, stride_xy, stride_z, patch_size, device,
...
def test_all_case_ISLES22(model, image_list, num_classes, patch_size=(96, 96, 64), stride_xy=16, stride_z=4,
                          save_result=True,
                          test_save_path=None, preproc_fn=None, metric_detail=0, nms=0):
    loader = tqdm(image_list) if not metric_detail else image_list
    total_metric = 0.0
...
def test_all_case(model, image_list, num_classes, device, patch_size=(112, 112, 80), stride_xy=18, stride_z=4,
                  save_result=True,
                  test_save_path=None, preproc_fn=None, metric_detail=0, nms=0):
    loader = tqdm(image_list) if not metric_detail else image_list
    total_metric = 0.0
...
def test_single_case(model, image, stride_xy, stride_z, patch_size, num_classes=1):
    ...
    label_map = np.argmax(score_map, axis=0)#(score_map[0] > 0.5).astype(int)
    if add_pad:
