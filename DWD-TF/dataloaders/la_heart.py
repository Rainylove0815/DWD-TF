...
class Pancras(Dataset):
    """ LA Dataset """
    def __init__(self, base_dir=None, base_dir_list = None, split='train',train_flod=None, common_transform=None,sp_transform=None):
        self._base_dir = base_dir
        ...
        print(train_flod)
        if split=='train':
            with open(self.base_dir_list+'/Flods/'+train_flod, 'r') as f:
                self.image_list = f.readlines()
...
class LAHeart(Dataset):
    ...
    def __init__(self, base_dir=None, split='train',num=None,transform=None):
        ...
        self.sample_list = []
        if split=='train':
            with open(self.base_dir+'\\Flods\\train0.list', 'r') as f:
                self.image_list = f.readlines()
        elif split=='test':
            with open(self.base_dir+'\\Flods\\test0.list', 'r') as f:
                self.image_list = f.readlines()
        self.image_list = [item.replace('\n','') for item in self.image_list]
        if num is not None:
            self.image_list = self.image_list[:num]
    ...
    def __getitem__(self, idx):
        image_name = self.image_list[idx]
        h5f = h5py.File(self.base_dir+"\\ladata\\"+image_name+"\\mri_norm2.h5", 'r') #wyc_mri_norm0_255
        image = h5f['image'][:]
        label = h5f['label'][:]
        sample = {'image': image, 'label': label}
        if self.transform:
