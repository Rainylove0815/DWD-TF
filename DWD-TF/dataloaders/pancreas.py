...
class Pancreas(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # h5f = h5py.File(self._base_dir + "/Pancreas_data/{}.h5".format(image_name), 'r')
        h5f = h5py.File(self._base_dir + "/Pancreas_data/{}".format(image_name), 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
...
if __name__ == "__main__":
    ...
    def plot_sample(sample):
        ...
        axes[1].imshow(label[:, :, image.shape[2] // 2], cmap='gray')
        axes[1].set_title('Label')
        plt.savefig('./sample_images/pancreas_sample.png')
        plt.close()
        # plt.show()


    base_dir = "../../data/Pancreas"
    patch_size = (96, 96, 96)
    data = Pancreas(base_dir=base_dir, split='train', transform=T.Compose([
        Resize(patch_size),
